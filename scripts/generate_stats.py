...
from sokoban_bazaar.dataset import get_dataset, pad_episodic_batch, _ENV_NAMES, _DATASET_NAMES, get_test_envs
                ...
                for file in tqdm(episode_dataset.episode_files):
                    try:
                        with open(file, 'rb') as _file:
                            ...
                            episode_score['max'] = max(episode_score['max'],
                                                       _episode_info['returns_to_go'][0])
                            episode_score['min'] = min(episode_score['min'],
                                                       _episode_info['returns_to_go'][0])
...
if __name__ == '__main__':
    ...
    print(markdownTable(stats_df.to_dict(orient='records'))
          ...
          .getMarkdown())
    print("\n\n")
    print(markdownTable(eval_df.to_dict(orient='records'))
          .setParams(row_sep='always')
          .getMarkdown())
