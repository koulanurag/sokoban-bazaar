...
def set_state(env, tiny_rgb_observation):
    ...
    player_position = np.argwhere(env.room_state == 5)  # player on floor cell
    if len(player_position) == 0:
        player_position = np.argwhere(env.room_state == 6)  # player on target
    env.unwrapped.player_position = player_position[0]
